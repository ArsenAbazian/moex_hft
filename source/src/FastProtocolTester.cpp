...
void FastProtocolTester::TestMessages() {
    ...
    message = new unsigned char[52] {
            0xc6, 0x2a, 0x00, 0x00, 0xe0, 0x12, 0xec, 0x55, 0xc6, 0x23,
            0x68, 0x07, 0x6d, 0x69, 0x1d, 0x01, 0xa5, 0x81, 0x80, 0x00,
            0xe5, 0x52, 0x50, 0x55, 0xcf, 0x52, 0x55, 0x30, 0x30, 0x30,
            0x41, 0x30, 0x4a, 0x56, 0x57, 0x41, 0xb5, 0x82, 0x82, 0x93,
            0x80, 0x81, 0xca, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
            0x80, 0x80 };

    manager->SetNewBuffer(message, 53);
    msgSeqNo = manager->ReadMsgSeqNumber();
    snap = (FastSnapshotInfo*)manager->GetSnapshotInfo();
}
